...
{
    public class AlphaSpTraceLogProvider : BaseStorageProvider
    {
        public override void AddEntry(LogEntry entry)
        {
            string tagString = entry.ProductName.Length >= 4 ? entry.ProductName.Substring(0, 4) : "Alph";
            uint tag = TraceProvider.TagFromString(tagString);
            TraceProvider.TraceSeverity level;

            switch (entry.Level)
            {
                case LogEntryLevel.Debugging:
                    level = TraceProvider.TraceSeverity.Assert;
                    break;
                case LogEntryLevel.Trace:
                    level = TraceProvider.TraceSeverity.Verbose;
                    break;
                case LogEntryLevel.Information:
                    level = TraceProvider.TraceSeverity.InformationEvent;
                    break;
                case LogEntryLevel.Warning:
                    level = TraceProvider.TraceSeverity.WarningEvent;
                    break;
                case LogEntryLevel.Error:
                    level = TraceProvider.TraceSeverity.Exception;
                    break;
                default:
                    level = TraceProvider.TraceSeverity.InformationEvent;
                    break;
        ...
        {
            ConfigurationManager.Logger.ProviderSettings[Properties.Settings.Default.KeyAlphaSpTraceLogProviderIsEnabled].Value = Enabled.ToString();
