<%@ Control Language="C#" Inherits="AlphaMosaik.Logger.Administration.WebControls.AlphaLogProviderSection, AlphaMosaik.Logger.Administration, Version=1.0.0.0, Culture=neutral, PublicKeyToken=e765a843b29600b9"
...
_spBodyOnLoadFunctionNames.push("AlphaLogProviderSection_OnLoad");

function AlphaLogProviderSection_OnLoad()
{
	InitAlphaLogProviderSectionControls(true);
}

function InitAlphaLogProviderSectionControls(bSilent)
{
    var bProviderEnabled = document.getElementById(<%SPHttpUtility.AddQuote(SPHttpUtility.NoEncode(ChkBoxAlphaLogProviderEnabled.ClientID),Response.Output);%>).checked;
    var radDefaultFilePath = document.getElementById(<%SPHttpUtility.AddQuote(SPHttpUtility.NoEncode(RadDefaultFilePath.ClientID),Response.Output);%>);
    var radCustomFilePath =  document.getElementById(<%SPHttpUtility.AddQuote(SPHttpUtility.NoEncode(RadCustomFilePath.ClientID),Response.Output);%>);
    var txBoxCustomFilePath = document.getElementById(<%SPHttpUtility.AddQuote(SPHttpUtility.NoEncode(TxBoxCustomFilePath.ClientID),Response.Output);%>);
    var reqValCustomFilePath = document.getElementById(<%SPHttpUtility.AddQuote(SPHttpUtility.NoEncode(ReqValCustomFilePath.ClientID),Response.Output);%>);
    
    radDefaultFilePath.disabled = !bProviderEnabled;
    radCustomFilePath.disabled = !bProviderEnabled;
    
    if(!bProviderEnabled || !radCustomFilePath.checked) {
        txBoxCustomFilePath.disabled = true;
    }
    else {
        txBoxCustomFilePath.disabled = false;
    }
    
    ValidatorEnable(reqValCustomFilePath, !txBoxCustomFilePath.disabled);
}

function ProviderEnabledChanged_<%=this.ClientID %>()
{
	InitAlphaLogProviderSectionControls();
}

function LogFileModeChanged_<%=this.ClientID%>()
{
    InitAlphaLogProviderSectionControls();
}
</script>
